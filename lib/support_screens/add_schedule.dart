...
class AddScheduleScreen extends StatelessWidget {
  ...
  AddScheduleScreen({required this.schedules});
  ...
  void addSchedule () async {
    ...
    final file = File('$localPath/schedule_data.json');

    if (!await file.exists()) {
      // read the file from assets first and create the local file with its contents
      final initialContent = await rootBundle.loadString("assets/events.json");
      await file.create();
      await file.writeAsString(initialContent);
      print(initialContent);
    }
